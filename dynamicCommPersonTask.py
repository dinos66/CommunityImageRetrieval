...
class communitydynamic:
    ...
    def from_txt(cls,timeSeg,filename,dataset_path_results,dataset_path_tmp):
        ...
        # stopNodes = ['entertainment_group','public_image_ltd','ricardo_tormo','graydon_carter','name_of_person','sugarland','the_black_eyed_peas','ed_sullivan',' ']
    ...
    def extraction(self, commDetectMethod, timeSelection):
        ...
        for tmplim in mentionLimit:
            ...
            # strComms = [inv_partitionLouv[x] for x in inv_partitionLouv]
    ...
    def timeslotselection(self, alltime, seg):
        ...
        ax.set_xticks(list(range(0, len(freqStat), pertick)))#, minor=False)
    ...
    def photoRetrieval(self, topImages, person, captiondict, decisionforAll):
        ...
        if decisionforAll == str('y') or decisionforAll == str(3):
            ...
            pertick = math.ceil(len(dateArray)/30)
            ax.set_xticks(list(range(0, len(dateArray), pertick)))#, minor=False)
            ax.set_xticklabels(self.day_month[0:-1:pertick], fontsize = 12, rotation = 35)# minor=False,
            ax.legend(loc='upper left', numpoints = 1)
            mng = plt.get_current_fig_manager()
            mng.resize(*mng.window.maxsize())
            plt.show()
            fig.savefig(self.dataset_path_results+self.fileTitle+'/userStats/'+person+".pdf",bbox_inches='tight', format='pdf')
            plt.close()
        ...
        # statement = 'Overlapping occurred.'
        # statsfile = open(self.dataset_path_results+self.fileTitle+'/'+ person +"/basicstats.txt",'w')
        # print(statement)
        # statsfile.write(statement)
        # statsfile.close()
        # nodeFile.close()
